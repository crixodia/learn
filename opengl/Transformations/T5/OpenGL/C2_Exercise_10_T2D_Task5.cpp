...
int main()
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load("textures/awesomeface.gif", &width, &height, &nrChannels, 0);    if (data)
    if (data)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float scale2 = cos(glfwGetTime());
        transform = glm::translate(transform, glm::vec3(1.0f*scale, 1.0f * scale2, 0.0f));
        ...
        scale = 0.02f*sin(glfwGetTime()) + 1.0f;
